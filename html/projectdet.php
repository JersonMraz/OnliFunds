<?php
    ...
    $fetchProject = "SELECT DATEDIFF(end_date, NOW()) AS remaining_days FROM projects";
    $stmt = $conn->prepare($fetchProject);
	$stmt->execute();
	$rem = $stmt->get_result();
    $remainingDay = $rem->fetch_assoc();

    $countProject = "SELECT (SELECT COUNT(proj_id) FROM projects WHERE user_id = p.user_id) AS projfound FROM projects p";
    $stmt = $conn->prepare($countProject);
    $stmt->execute();
    $countResult = $stmt->get_result();
    $projfound = $countResult->fetch_assoc();

    $fetchProject = "SELECT p.proj_id, p.proj_title, p.category, p.city, p.author, p.fundgoal, p.project_overview, p.project_photo, p.author_icon, p.profile_pic, DATEDIFF(p.end_date, NOW()) AS remaining_days, (SELECT COUNT(proj_id) FROM projects WHERE user_id = p.user_id) AS projfound FROM projects p";
    $stmt = $conn->prepare($fetchProject);
	$stmt->execute();
	$result = $stmt->get_result();
    ...
    $fetchDonation = "SELECT SUM(donation_amount) AS total_donated FROM donation WHERE proj_id = ?";
    $stmt = $conn->prepare($fetchDonation);
    $stmt->bind_param("i", $project_id);
    $stmt->execute();
    $donationResult = $stmt->get_result();
    $donation = $donationResult->fetch_assoc();

    $totalDonated = $donation['total_donated'] ? floatval($donation['total_donated']) : 0;
    $fundGoal = str_replace(',', '', $project['fundgoal']);
    $fundGoal = floatval($fundGoal);

    $progressPercentage = ($fundGoal > 0) ? ($totalDonated / $fundGoal) * 100 : 0;
...
<body>
        ...
        <div class="project-header">
            <div style="display: flex; flex-direction: column;">
                <div id="carouselExample" class="carousel slide">
                    <div class="carousel-inner">
                        <div class="carousel-item active">
                            <img src="../images/<?php echo htmlspecialchars($project['project_photo']) ?>" class="d-block" alt="...">
                        </div>
                ...
                <div class="thumbnail-images">
                    <img src="../images/<?php echo htmlspecialchars($project['project_photo']) ?>" alt="Thumbnail" class="thumbnail">
                    <img src="../images/<?php echo htmlspecialchars($project['project_photo']) ?>" alt="Thumbnail" class="thumbnail">
                    <img src="../images/<?php echo htmlspecialchars($project['project_photo']) ?>" alt="Thumbnail" class="thumbnail">
                    <img src="../images/<?php echo htmlspecialchars($project['project_photo']) ?>" alt="Thumbnail" class="thumbnail">
                </div>
            ...
            <div class="project-info">
                <div class="project-author" style="margin-top: 0; margin-bottom: 20px;">
					<div class="detuser">
                    ...
                    <div class="author-info">
                        ...
                        <span>
                            <p><?php echo htmlspecialchars($projfound['projfound']) ?> Campaigns</p> | Cebu City
                        </span>
                    </div>
					</div>
					<div class="heartflag">
						<button style="margin-right: 10px;"><i class="fa fa-share-alt" aria-hidden="true"></i></button>	
						<button><a href="" class="showOver"><i class="fas fa-flag"></i></a></button>
					</div>
                </div>
                <p class="category"><?php echo htmlspecialchars($project['category']) ?></p>
                <h2><?php echo htmlspecialchars($project['proj_title']) ?></h2>
                <p class="description">
                    <?php echo htmlspecialchars($project['project_overview']) ?>
                </p>
                <div class="custom-progress-container">
                    <!-- Progress Bar -->
                    <div class="custom-progress w-100 position-relative" style="height: 10px; border-radius: 5px;">
                        <div class="custom-progress-bar" role="progressbar" style="width: <?php echo round($progressPercentage, 2); ?>%;" aria-valuenow="<?php echo round($progressPercentage, 2); ?>" aria-valuemin="0" aria-valuemax="100">
                            <!-- Percentage Label -->
                            <span class="custom-progress-label"><?php echo round($progressPercentage, 2); ?>%</span>
                        </div>
                    </div>
                </div>
            <div class="m-p-d-info-part-3">
                <div class="dets-cont">
                    <img src="../images/fundgoal.svg" alt="">
                ...
                <div class="dets-cont">
                    <img src="../images/moneylogo.svg" alt="">
                    <div class="dets-conts-2">
                        <h1>Php <?php echo $totalDonated ?></h1>
                        <p>Total Raised</p>
                    </div>
                </div>
                <div class="dets-cont">
                    ...
                    <div class="dets-conts-2">
                        <h1><?php echo htmlspecialchars($project['remaining_days']) ?></h1>
                        <p>Days to go</p>
                    </div>
                </div>

            </div>
            <button class="fund-button" onclick="window.location.href='donate.php?proj_id=<?php echo htmlspecialchars($project['proj_id']); ?>'">Fund This Project</button>
            </div>
        </div>
    </section>
 ...
 <section class="about-project">
    <div class="about-proj-cont" style="width:100%;">
        <div class="about-proj-list" id="menu">
            <button id="menu-about" onclick="showSection('about', this)">About Project</button>
            <button onclick="showSection('updates', this)">Updates</button>
            <button onclick="showSection('support', this)">Supporter List</button>
            <button onclick="showSection('comments', this)">Comments</button>
        </div>

        <!-- ABOUT DIV -->
        <div id="about" class="content-section active">
            <img src="../images/<?php echo htmlspecialchars($project['project_photo']) ?>" alt="" class="aboutpic">
            <p><?php echo htmlspecialchars($project['project_story']) ?></p>
        </div>
...
<div id="updates" class="content-section">
    <div class="timeline">
      <div class="timeline-item">
        ...
        <div class="timeline-content">
          ...
          <a href="" class="showOverlayLink">See update</a>
        </div>
      </div>
      <div class="timeline-item">
        <div class="timeline-icon">
        ...
        <div id="support" class="content-section">
            <div class="support-cont">
                <div class="support-info">
                    <div class="supporter-name namepic" >
						<div class="supporter-img">
							<div class="author-icon">AG</div>
						</div>
						<div>
							<h1>Adelina Dela Crus</h1>
							<h1>Donated on November 10, 2024</h1>
						</div>
                    </div>
          ...
          <div id="comments" class="content-section">
            <div class="comments-cont">
                <div class="com-header">
                    <h1>Only supporter can post comments.</h1>
                </div>
                <div class="com-text-box">
                    <div class="com-pfp">
                        <img src="../images/profile.png" alt="" class="compic">
                        <h1>John Doe</h1>
                    </div>

                    <div  class="text-box" id="editor" contenteditable="true" style="padding: 10px; min-height: 100px;">
						<!-- Content goes here -->
					</div>
                    <div class="com-but-list">
						<div class="toolbar">
							<button class = "comtool" onclick="toggleBold()"><i class="fas fa-bold"></i></button>
							<button class = "comtool" onclick="toggleItalic()"><i class="fas fa-italic"></i></button>
                        </div>
    ...
    function toggleItalic() {
        document.execCommand('italic', false, null);
                ...
                <div class="comment-hist">
                    <div class="comment-hist-cont">
						<div class="supporter-img">
							<div class="author-icon">AG</div>
						</div>
                        <div class="comment-hist-info">
                            <h1>Adelina Dela Crus</h1>
                            <h2>17 hrs ago</h2>
                            <h3>God bless you Rosita for helping those animals in need.</h3>
                        </div>
                    </div>
                    <div class="comment-hist-cont">
						<div class="supporter-img">
    ...
    <section class="support-projects">
        <span class="support">Support other projects</span>
            <?php foreach($rows as $row): ?>
                <div class="projectimg">
                    <img src="../images/<?php echo htmlspecialchars($row['project_photo']) ?>" alt="Tarsier" class="picture" />
                </div>
                <span class="project">
                    <?php echo htmlspecialchars($row['proj_title']) ?>
                </span>
                    <div class="over"><span><?php echo htmlspecialchars($row['project_overview']) ?></span></div>
                <div class="project-autho">
                    ...
                    <div class="author-info">
                        ...
                        <span>
                            <p style="margin: 0;"><?php echo htmlspecialchars($row['projfound']) ?> Campaigns</p> | Cebu City
                        </span>
                    </div>
                </div>
            <?php endforeach; ?>
    ...
    <section class="banner-section">
        ...
        <div class="banner-content">
            ...
            <a href="projects.html" class="donate-button" style="font-weight:bold;">DONATE</a>
        </div>
    </section>
